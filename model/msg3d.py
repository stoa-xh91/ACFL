...
from model.msg3d_helper.ms_gcn import MultiScale_GraphConv as MS_GCN
from model.msg3d_helper.ms_tcn import MultiScale_TemporalConv as MS_TCN
from model.msg3d_helper.ms_gtcn import SpatialTemporal_MS_GCN, UnfoldTemporalWindows
from model.msg3d_helper.mlp import MLP
from model.msg3d_helper.activation import activation_factory
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = out.mean(1)   # Average pool number of bodies in the sequence

        out = self.fc(x)
        return {'logits':out, 'feature':x}
...
class NaiveMultiModel(nn.Module):
    def __init__(self,
        ...
        self.data_bn = nn.BatchNorm1d(num_person * 2*in_channels * num_point)
        ...
        # r=3 STGC blocks
        self.gcn3d1 = MultiWindow_MS_G3D(6, c1, A_binary, num_g3d_scales, window_stride=1)
        self.sgcn1 = nn.Sequential(
            MS_GCN(num_gcn_scales, 6, c1, A_binary, disentangled_agg=True),
            MS_TCN(c1, c1),
            MS_TCN(c1, c1))
        self.sgcn1[-1].act = nn.Identity()
        self.tcn1 = MS_TCN(c1, c1)
